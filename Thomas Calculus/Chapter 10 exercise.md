...
19. $\sum\limits_{n = 3}^{\infty}\dfrac{1}{(2n - 3)(2n - 1)}$  
    $\dfrac{1}{(2n - 3)(2n - 1)} = \dfrac{1}{2}(\dfrac{1}{2n - 3} - \dfrac{1}{2n - 1})$  
    $s_n = \dfrac{1}{2}(\dfrac{1}{3} - \dfrac{1}{5} + \dfrac{1}{5} - \dfrac{1}{7} + \cdots + \dfrac{1}{2n - 3} - \dfrac{1}{2n - 1})$  
    $= \dfrac{1}{2}(\dfrac{1}{3} -\dfrac{1}{2n - 1})$  
    $\sum\limits_{n = 3}^{\infty}\dfrac{1}{(2n - 3)(2n - 1)} = \dfrac{1}{6}$
...
25. $\sum\limits_{n = 1}^{\infty}\dfrac{1}{\sqrt{n}}$   
    $f(x) = \dfrac{1}{\sqrt{x}}$ is continuous, positive, decreasing  
    $\int_1^{\infty}\dfrac{1}{\sqrt{x}}$  
    $=\lim\limits_{b\to\infty}[\dfrac{2}{3}b^{\frac{3}{2}} - \dfrac{2}{3}]$, diverges  
    The integral test gives that the series diverges. 
#### Power Series
In Exercises 45–54, (a) find the series’ radius and interval of convergence. Then identify the values of $x$ for which the series converges (b) absolutely and (c) conditionally.

45. $\sum\limits_{n = 1}^{\infty}\dfrac{(x + 4)^n}{n3^n}$  
    $\lim\limits_{n\to\infty}|\dfrac{\frac{(x + 4)^{n + 1}}{(n + 1)3^{n + 1}}}{\frac{(x + 4)^n}{n3^n}}|$  
    $= \lim\limits_{n\to\infty}|\dfrac{(x + 4)n}{3(n + 1)}|$  
    $|\dfrac{x + 4}{3}| < 1. -7 < x < -1$  
    When $x = -7,$ the alternating series test gives that the series converges  
    $\sum\limits_{n = 1}^{\infty}\dfrac{1}{n}$ diverges  
    $R: 3, I: [-7,-1)$  
    conditionally converges at $x = -7$
#### Maclaurin Series
Each of the series in Exercises 55–60 is the value of the Taylor series at $x = 0$ of a function $f(x)$ at a particular point. What function and what point? What is the sum of the series?

55. $1 -\dfrac{1}{4} + \dfrac{1}{16} - \cdots + (-1)^n\dfrac{1}{4^n} + \cdots$  
    $\dfrac{1}{1 +x}$ at $x = \dfrac{1}{4}$  
    $\dfrac{4}{5}$
#### Taylor Series
In Exercises 69–72, find the first four nonzero terms of the Taylor series generated by $f$ at $x = a$.

69. $f(x) = \sqrt{3 + x^2}$ at $x = -1$  
    $f'(x) = \dfrac{x}{\sqrt{3 + x^2}}$  
    $f''(x) = \dfrac{\sqrt{3 + x^2}- \frac{x^2}{\sqrt{3 + x^2}}}{3+ x^2}=\dfrac{\frac{3}{\sqrt{3 + x^2}}}{3 + x^2}= \dfrac{3}{(3 + x^2)\sqrt{3 + x^2}}$    
    $f'''(x) = \dfrac{-3\frac{3}{2}\sqrt{3 + x^2}2x}{(3 + x^2)^3} = -\dfrac{9x\sqrt{3 + x^2}}{(3 + x^2)^3}$  
    $2 -\dfrac{1}{2}(x + 1) +\dfrac{3}{16}(x + 1)^2 + \dfrac{3}{64}(x + 1)^3$  
#### Nonelementary Integrals
Use series to approximate the values of the integrals in Exercises 73–76 with an error of magnitude less than $10^{-8}$. (The answer section gives the integrals’ values rounded to 10 decimal places.)

73. $\int_0^{\frac{1}{2}}e^{-x^3}dx$  
    $e^x = 1 + x + \dfrac{x^2}{2!} + \dfrac{x^3}{3!} + \dfrac{x^4}{4!} - \dfrac{x^5}{5!} + \cdots$  
    $e^{-x^3}= 1-x^3 + \dfrac{x^6}{2!} - \dfrac{x^9}{3!} + \dfrac{x^{12}}{24} - \dfrac{x^15}{120} + \cdots$  
    $\int e^{-x^3}dx = C + x - \dfrac{1}{4}x^4 + \dfrac{1}{14}x^7 - \dfrac{1}{60}x^{10} + \dfrac{1}{312}x^{13} - \dfrac{1}{1920}x^{16} + \cdots$  
    $\int_0^{\frac{1}{2}}e^{-x^3}dx =  \dfrac{1}{2} - \dfrac{1}{64} + \dfrac{1}{1792} - \dfrac{1}{61440} + \dfrac{1}{2555904} - \dfrac{1}{125829120} + \cdots$  
    $\dfrac{1}{125829120} \approx 7.9 * 10 ^{-9} < 10 ^{-8}$  
    $\int_0^{\frac{1}{2}}e^{-x^3}dx \approx 0.4849171509$
#### Using Series to Find Limits
In Exercises 77–82:  
a. Use power series to evaluate the limit.  
b. Then use a grapher to support your calculation.

77. $\lim\limits_{x\to0}\dfrac{7\sin x }{e^{2x} - 1}$   
    $\dfrac{7\sin x }{e^{2x} - 1}$  
    $= \dfrac{7(x - \frac{x^3}{3!} + \frac{x^5}{5!} - \cdots)}{1 + 2x + \frac{4x^2}{2!} + \frac{8x^3}{3!} + \cdots -1}$   
    $\lim\limits_{x\to0}\dfrac{7\sin x }{e^{2x} - 1} = \dfrac{7}{2}$
#### Theory and Examples
83. Use a series representation of $\sin 3x$ to find values of $r$ and $s$ for which  
    $$
    \lim\limits_{x\to0}(\dfrac{\sin 3x}{x^3} + \dfrac{r}{x^2} + s) = 0
    $$
    $\sin 3x = 3x - 9x^3 + \dfrac{243x^5}{5!} - \cdots$  
    $\dfrac{\sin 3x}{x^3} = \dfrac{3}{x^2} - 9 + \dfrac{243x^2}{5!} - \cdots$  
    $r = -3, s = 9$
### Additional and Advanced Exercises
#### Determining Convergence of Series
Which of the series $\sum\limits_{n = 1}^{\infty}a_n$ defined by the formulas in Exercises 1–4 converge, and which diverge? Give reasons for your answers.
1. $\sum\limits_{n = 1}^{\infty}\dfrac{1}{(3n - 2)^{n + \frac{1}{2}}}$    
   $\lim\limits_{x\to\infty}\sqrt[n]{\dfrac{1}{(3n - 2)^{n + \frac{1}{2}}}}$  
   $= \lim\limits_{x\to\infty}\dfrac{1}{(3n -2)\sqrt[2n]{3n -2}} = 0 < 1$  
   The root test gives that the series converges
#### Choosing Centers for Taylor Series
Taylor’s formula
$$
f(x) = f(a) + f'(a)(x-a) + \dfrac{f''(a)}{2}(x-a)^2 + \cdots + \dfrac{f^{(n)}(a)}{n!}(x-a)^n + \dfrac{f^{(n + 1)}(a)}{(n + 1)!}(x-a)^{n + 1}
$$
expresses the value of $f$ at $x$ in terms of the values of $f$ and its derivatives at $x = a$. In numerical computations, we therefore need $a$ to be a point where we know the values of $f$ and its derivatives. We also need $a$ to be close enough to the values of $f$ we are interested in to make $(x-a)^{n + 1}$ so small we can neglect the remainder.  
...
9. $\cos x$ near $x = 1$  
   $a = \dfrac{\pi}{3}$  
   $\cos 1 \approx 1 - \dfrac{(x -\dfrac{\pi}{3})^2}{2} + \dfrac{(x - \dfrac{\pi}{3})^4}{4!} - \dfrac{(x - \dfrac{\pi}{3})^6}{6!}$
#### Theory and Examples
15. Let $a$ and $b$ be constants with $0 < a < b$. Does the sequence $\{(a^n + b^n)^{\frac{1}{n}}\}$ converge? If it does converge, what is the limit?  
    $(b^n)^{\frac{1}{n}} < (a^n + b^n)^{\frac{1}{n}} < (b^n + b^n)^{\frac{1}{n}}$   
    $(b^n + b^n)^{\frac{1}{n}} = 2^{\frac{1}{n}}b$   
    $\lim\limits_{n\to\infty}(b^n)^{\frac{1}{n}} = \lim\limits_{n\to\infty}(b^n + b^n)^{\frac{1}{n}} = b$  
    The sandwich theorem gives: $\lim\limits_{n\to\infty}(a^n + b^n)^{\frac{1}{n}} = b$